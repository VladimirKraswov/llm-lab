...
const { startRuntime, stopRuntime } = require('../services/runtime');
...
router.post('/vllm/stop', async (_req, res) => {
  res.json({ ok: true, runtime: await stopRuntime() });
});
...
router.post('/use-job-output', async (req, res) => {
  try {
    ...
    const runtimeModelPath = job.modelId
      ? (await getModelById(job.modelId))?.path || job.baseModel
      : job.baseModel;
...
router.post('/chat', async (req, res) => {
  try {
    ...
    const supportsStreaming = !!state.capabilities?.supportsStreaming;
    ...
    const response = await provider.chat(state, payload);

    if (!response.ok) {
      const text = await response.text();
      return res.status(500).json({
        error: text || 'Inference failed',
        provider: providerId,
      });
    }
